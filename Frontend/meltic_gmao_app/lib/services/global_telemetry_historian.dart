  ...
  Future<void> _pollAll() async {
    ...
    try {
      final futures = _trackedMaquinas.where((m) => m.id != null).map((m) async {
        ...
        try {
          // Si no tenemos nada, solo pedimos lo último para el poller (la carga inicial es aparte)
          if (since == null) {
            // Carga inicial ligera (últimos 10 puntos) para dar feedback rápido
            final data = await _service.fetchPorMaquina(id); 
            // En un historiador real, aquí pediríamos poco si es background progresivo
            _addAllToBuffer(id, data.take(10).toList());
          } else {
