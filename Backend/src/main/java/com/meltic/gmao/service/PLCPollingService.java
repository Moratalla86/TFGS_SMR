...
public class PLCPollingService {
    ...
    // Cache del último estado conocido de cada máquina: evita queries innecesarias a MySQL
    private final java.util.Map<Long, String> machineStateCache = new ConcurrentHashMap<>();
        ...
        try {
            Telemetria t = new Telemetria();
                ...
                if (forcedSimulation) {
                    ...
                    t.setHumedad(40.0 + ThreadLocalRandom.current().nextDouble() * 20.0);
                    t.setVibracion(1.0 + ThreadLocalRandom.current().nextDouble() * 2.0);
    ...
    public void procesarAlertas(Telemetria t) {
        ...
        if (maquinaOpt.isPresent()) {
            ...
            String newState = alarmaDetectada != null
                    ? (alarmaDetectada.contains("MUY") ? "ERROR" : "WARNING")
                    : "OK";
            if (alarmaDetectada != null) {
                t.setAlarma(alarmaDetectada);
            }
            // Solo guardar en MySQL si el estado cambió realmente (evita queries innecesarias)
